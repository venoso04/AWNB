...
const passwordResetSchema = new mongoose.Schema({
  ...
  // Set to true once the code has been verified, allowing the
  // final "set new password" step without re-entering the code.
  verified: {
    type: Boolean,
    default: false,
  },
...
passwordResetSchema.statics.hashCode = async (rawCode) => {
  const salt = await bcrypt.genSalt(10);
  return bcrypt.hash(rawCode, salt);
};

// ─── Compare a candidate code against the stored hash ────
passwordResetSchema.methods.verifyCode = async function (candidateCode) {
